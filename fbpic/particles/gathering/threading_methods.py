...
from .inline_functions import (
    ...
    add_cubic_gather_for_mode,
    add_linear_gather_for_mode_scalar,
    add_cubic_gather_for_mode_scalar )
...
add_linear_gather_for_mode_scalar = numba.njit( add_linear_gather_for_mode_scalar )
add_cubic_gather_for_mode_scalar = numba.njit( add_cubic_gather_for_mode_scalar )
...
@njit_parallel
def gather_scalar_numba_linear(x, y, z,
                    rmax_gather,
                    invdz, zmin, Nz,
                    invdr, rmin, Nr,
                    Ez_m0,
    ...
    Ez_m0 : 2darray of complexs
        The electric fields on the interpolation grid for the mode 0

    Ez_m1 : 2darray of complexs
        The electric fields on the interpolation grid for the mode 1

    Ez : 1darray of floats
        The electric fields acting on the particles
        (is modified by this function)
    ...
    for i in prange(x.shape[0]):
        ...
        if rj < rmax_gather:
            # Add contribution from mode 0
            Fz = add_linear_gather_for_mode_scalar( 0,
                Fz, exptheta_m0, Ez_m0,
                iz_lower, iz_upper, ir_lower, ir_upper,
                S_ll, S_lu, S_lg, S_ul, S_uu, S_ug )
            # Add contribution from mode 1
            Fz = add_linear_gather_for_mode_scalar( 1,
                Fz, exptheta_m1, Ez_m1,
                iz_lower, iz_upper, ir_lower, ir_upper,
                S_ll, S_lu, S_lg, S_ul, S_uu, S_ug )
        # Convert to Cartesian coordinates
        # and write to particle field arrays
...
@njit_parallel
def gather_scalar_numba_cubic(x, y, z,
                    rmax_gather,
                    invdz, zmin, Nz,
                    invdr, rmin, Nr,
                    Ez_m0,
    ...
    for nt in prange( nthreads ):
        ...
        for i in range( ptcl_chunk_indices[nt],
            ...
            if rj < rmax_gather:
                # Add contribution from mode 0
                Fz = add_cubic_gather_for_mode_scalar( 0,
                    Fz, exptheta_m0, Ez_m0,
                    ir_lowest, iz_lowest, Sr, Sz, Nr, Nz )
                # Add contribution from mode 1
                Fz = add_cubic_gather_for_mode_scalar( 1,
                    Fz, exptheta_m1, Ez_m1,
                    ir_lowest, iz_lowest, Sr, Sz, Nr, Nz )
            # Convert to Cartesian coordinates
            # and write to particle field arrays
