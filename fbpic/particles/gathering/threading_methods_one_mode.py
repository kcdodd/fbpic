...
@njit_parallel
def gather_scalar_numba_linear_one_mode(x, y, z,
                    rmax_gather,
                    invdz, zmin, Nz,
                    invdr, rmin, Nr,
                    Ez_m, m,
    ...
    for i in prange(x.shape[0]):
        ...
        if rj < rmax_gather:
            ...
            Fz = 0.
            # Add contribution from mode m
            Fz = add_linear_gather_for_mode_scalar( m,
                Fz, exptheta_m, Ez_m,
                iz_lower, iz_upper, ir_lower, ir_upper,
                S_ll, S_lu, S_lg, S_ul, S_uu, S_ug )
            # Convert to Cartesian coordinates
...
@njit_parallel
def gather_scalar_numba_cubic_one_mode(x, y, z,
                    rmax_gather,
                    invdz, zmin, Nz,
                    invdr, rmin, Nr,
                    Ez_m, m,
    ...
    for nt in prange( nthreads ):
        ...
        for i in range( ptcl_chunk_indices[nt],
            ...
            if rj < rmax_gather:
                ...
                Fz = 0.
                # Add contribution from mode m
                Fz = add_cubic_gather_for_mode_scalar( m,
                    Fz, exptheta_m, Ez_m,
                    ir_lowest, iz_lowest, Sr, Sz, Nr, Nz )
                # Convert to Cartesian coordinates
                # and write to particle field arrays
